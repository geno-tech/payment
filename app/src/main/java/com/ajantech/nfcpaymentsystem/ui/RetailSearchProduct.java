...
public class RetailSearchProduct extends Activity implements OnNFCServiceCallBack {
		...
		public void handleMessage(Message msg) {
			if (msg.what == 1025) {
				...
				case DefaultRequestStruct.REQUEST_TYPE_SEARCH_FOR_GOODS: {
					...
					ArrayList<TradeData> list = searchGoodsRequestStruct.getTradeList();
					getProductList(list);
					break;
				}
				default: {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mBaseAdapter = new SearchListBaseAdapter(this, mProductPrice, mProductCnt, mProductDate);
	...
	protected Dialog onCreateDialog(int id) {
		...
		case DLG_DATE: {
			...
			mTimeDialog.setOnCancelListener(new DialogInterface.OnCancelListener() {
	...
	private void searchGoods() {
		...
		CommunicationService.getInstance().searchGoods(
				...
				mProductEdittext.getText().toString(), mStartDateBtn.getText().toString(), mEndDateBtn.getText().toString());
	}
